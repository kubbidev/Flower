...
import me.kubbidev.flower.config.generic.key.ConfigKey;
import me.kubbidev.flower.config.generic.key.SimpleConfigKey;
import me.kubbidev.flower.storage.StorageType;
import me.kubbidev.flower.storage.implementation.split.SplitStorageType;
import me.kubbidev.flower.storage.misc.StorageCredentials;
...
import static me.kubbidev.flower.config.generic.key.ConfigKeyFactory.*;

/**
 * All of the {@link ConfigKey}s used by Flower.
 *
...
public final class ConfigKeys {
    ...
    public static final ConfigKey<String> SQL_TABLE_PREFIX = notReloadable(key(c -> {
        return c.getString("data.table-prefix", c.getString("data.table_prefix", "flower_"));
    }));
    ...
    /**
     * # If the servers own UUID cache/lookup facility should be used when there is no record for a player in the Flower cache.
     */
