...
public abstract class ParentCommand<T, I> extends Command<Void> {
    ...
    @Override
    public void execute(FlowerPlugin plugin, Sender sender, Void ignored, ArgumentList args, String label) throws CommandException {
    ...
    @Override
    public List<String> tabComplete(FlowerPlugin plugin, Sender sender, ArgumentList args) {
        switch (this.type) {
    ...
    protected abstract List<String> getTargets(FlowerPlugin plugin);

    protected abstract I parseTarget(String target, FlowerPlugin plugin, Sender sender);

    protected abstract ReentrantLock getLockForTarget(I target);

    protected abstract T getTarget(I target, FlowerPlugin plugin, Sender sender);

    protected abstract void cleanup(T t, FlowerPlugin plugin);

    public enum Type {
        // e.g. /flower log sub-command....
        NO_TARGET_ARGUMENT(0),
        // e.g. /flower user <USER> sub-command....
