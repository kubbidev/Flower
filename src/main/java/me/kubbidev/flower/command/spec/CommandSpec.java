package me.kubbidev.flower.command.spec;
...
public enum CommandSpec {
    ...
    CommandSpec(String usage, PartialArgument... args) {
        ...
        this.args = args.length == 0 ? null : Arrays.stream(args)
                .map(builder -> {
                    ...
                    TranslatableComponent description = Component.translatable("flower.usage." + key() + ".argument." + key);
    ...
    public TranslatableComponent description() {
        return Component.translatable("flower.usage." + this.key() + ".description");
    }
