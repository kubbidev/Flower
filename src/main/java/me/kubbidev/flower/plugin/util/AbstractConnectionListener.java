package me.kubbidev.flower.plugin.util;
...
public abstract class AbstractConnectionListener {
    ...
    protected AbstractConnectionListener(FlowerPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public User loadUser(UUID uniqueId, String username) {
        ...
        if (saveResult.includes(PlayerSaveResult.Outcome.OTHER_UNIQUE_IDS_PRESENT_FOR_USERNAME)) {
            ...
            this.plugin.getLogger().warn("Flower already has data for player '" + username + "' - but this data is stored under a different UUID.");
